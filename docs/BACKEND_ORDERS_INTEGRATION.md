...
| **POST** | **`/orders`** | **Staff** | **Create order manually (Sales page). Body: see §2.1. Returns created order with status `NEW`. Emit `order_created` so Delivery and Order Tracking update.** |
| PATCH | `/orders/:id/status` | Staff | Update order status (body: `{ "status": "PROCESSING" \| "DISPATCHED" \| "DELIVERED" }`). |

### 2.1 POST /orders — create order (Sales page)

Staff can add a new purchase manually from the **Sales** page. The frontend sends:
...
- **Required:** `customer_name`, `mobile`, `order_date`, `items` (at least one item with `name`, `quantity`, `price`).
- **Optional:** `customer_id` (positive integer; if provided, link order to this existing customer; if omitted, create a new customer — see below). Also optional: `address`, `delivery_due_date`; per item: `sku_color`, `size`, `product_id`.

Backend should:

1. **If `customer_id` is provided:** link the order to that existing customer.
2. **If `customer_id` is omitted:** create a **new customer** from the payload (`customer_name`, `mobile`, `address`, optionally `membership_level`). Insert into your PostgreSQL customer table, get the new customer **id**, attach that id to the mobile number, then link the order to that new id.
3. **Create the order** with status **`NEW`**. Generate `order_number`, `created_at`, `updated_at`, and persist in PostgreSQL.
4. Return the full created **Order** object (same shape as §3.2, including `customer_id`).
5. Emit the **`order_created`** Socket.io event so the Delivery and Order Tracking pages refresh and show the new order.
